...
parse s = (fst result, (init splits, snd result))
  where
    ...
    splits =
      (\case [amount, chemical] -> (chemical, read amount))
        <$> words
        <$> split splitter s
    result = last splits

produceAndReduce :: Reactions -> Requirements -> Requirements
produceAndReduce reactions =
...
produce reactions (chemical, amount)
  | amount > 0,
    ...
    let (times, leftover) = reactionCount amount producable
     in (chemical, - leftover) :
...
reactionCount required producible
  ...
  | leftover == 0 = (times, 0)
  where
    (times, leftover) = divMod required producible
...
ore m
  | M.size m' == 1, Just amount <- m' M.!? "ORE" = amount
...
binarySearchWith minBound maxBound f cmp
  ...
  | otherwise =
    let middle = div (minBound + maxBound) 2
     in case cmp $ f middle of
