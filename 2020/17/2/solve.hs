...
indexed4 :: [[Char]] -> [(Vec, Char)]
indexed4 = map (\((x, y), c) -> ((x, y, 0, 0), c)) . indexed2
...
dirs :: [Vec]
dirs = [(x, y, z, z') | x <- [-1 .. 1], y <- [-1 .. 1], z <- [-1 .. 1], z' <- [-1 .. 1], not $ (x == 0) && (y == 0) && (z == 0) && (z' == 0)]

add :: Vec -> Vec -> Vec
add (x1, y1, z1, z1') (x2, y2, z2, z2') = (x1 + x2, y1 + y2, z1 + z2, z1' + z2')
...
main :: IO ()
main = do
  print $ solve $ fromList $ indexed4 input
